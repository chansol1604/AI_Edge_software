...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  if(huart->Instance == USART2)
  {
    if(rxData == '\n')
    {
      buff[buffIndex]=0;
      char* p;
      if((p = strstr((char*)buff,"SetTime"))!=NULL)
      {
...
int main(void)
{
  ...
  while (1)
  {
    ...
    sprintf(temp, "\r20%02x-%02x-%02x %s %02x:%02x:%02x",sDate.Year, sDate.Month, sDate.Date, tFormat[sTime.TimeFormat], sTime.Hours, sTime.Minutes, sTime.Seconds);
